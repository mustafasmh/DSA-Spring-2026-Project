...
tk.Label(header, text="◈ POINTLIST", bg=BG, fg=ACCENT,
...
now_label = tk.Label(now_bar, text="♪  Nothing playing",
                     bg=NOW_BG, fg=ACCENT, font=FONT_HEAD)
...
def refresh():
    ...
    if playlist["size"] == 0:
        tk.Label(list_frame, text="Playlist is empty",
                 bg=BG, fg=SUBTEXT, font=FONT_MAIN).pack(pady=20)
        return
    ...
    while True:
        ...
        is_now = (playlist["now_playing"] is not None and
                  playlist["now_playing"] is current)

        row_bg  = ACCENT  if is_now else (ROW_A if number % 2 == 0 else ROW_B)
        row_fg  = BG      if is_now else TEXT

        row = tk.Label(list_frame, text=text, bg=row_bg, fg=row_fg,
                       font=FONT_NOW, anchor="w", padx=6, pady=5)
        row.pack(fill="x")
...
def do_artist():
    ...
    tk.Label(win, text=artist, bg=BG, fg=ACCENT,
             font=FONT_HEAD).pack(pady=10)
    ...
    while True:
        ...
        mins  = song["duration"] // 60
        secs  = song["duration"] % 60
        text  = (f"  {number}.  {song['title']:<38}"
                 f"{song['genre']:<12}{mins}:{secs:02d}   ▶ {song['play_count']}")
        tk.Label(frame, text=text, bg=ROW_A if number%2==0 else ROW_B,
                 fg=TEXT, font=FONT_NOW, anchor="w", padx=6, pady=5).pack(fill="x")
    ...
    def play_from_artist():
        ...
        if num is None or num < 1 or num > len(nodes): return
        node = nodes[num - 1]
        _play_node(node)
        win.destroy()
...
def do_play():
    num = ask("Enter song number to play:", int)
    if num is None or num < 1 or num > playlist["size"]: return
    current = playlist["head"]
